<?php
    ...
    $exam1 = App\Models\CandidateExam::where('candidate_id',auth()->user()->id)->where('exam_id',$exam->id)->orderBy('candidate_exams.id','desc')->get();

  if($exam1[0]->exam_take_status == "Taken"){
      header('Location: http://www.cplussoft.com/projects/EMS/public/candidate/exam-history');
  }
  else
  {}
...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    ?>
        <div class="min-h-screen bg-gray-100">
            ...
            <!-- Page Content -->
            <main>
            <form id="MyForm" name="MyForm" action="{{ route('candidate.exam.finish') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <?php $i = 1; $j = 0;?>
                <div class="container">
                    <div class="top_bar">
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <div class="row">
                                    <div class="col-md-6">
                                        <span class="float-left" id="total_time"></span>
                                    </div>
                                    <div class="col-md-2">
                                        <img class="float-right" src="{{ asset('/images/clock.jpg') }}" height="30" width="30">
                                    </div>
                                    <div class="col-md-4">
                                        <span class="float-left" id="timer"></span>
                                    </div>
                                </div>
                            </div>
                        </div>
                                ...
                                @foreach($questions as $question)
                                    @if($i <= count($questions))
                                        <div class="tab-pane m-t-10 fade card p-5 tab-class" name="tab_name" id="tab<?php echo $i; ?>" >
                                                @php $question_options = App\Models\QuestionOption::where('question_id',$question->id)->get(); @endphp
                                            <div class="row">
                                                ...
                                                <div class="col-12">
                                                    ...
                                                    <div class="col-6">
                                                    <input type="radio" name="option[<?php echo $j ?>]" onclick="saveAnswers({{$j}})" value="{{$question_option->option}}">
                                                    <!--<input type="hidden" name="option[]" id="q_option" value="">-->

                                                    {!! html_entity_decode($question_option->option) !!}
                                                    </div>
                                                    @endforeach
                                                </div>
                                                <ul class="list-inline wizard mb-0 mt-5">
                                                    @if($i != 1 && count($questions) >= 2)
                                                    <li class="previous list-inline-item ml-5"><a href="#" onclick="preTab(<?php echo $i ?>)"  id="previous" class="btn btn-info mb-0 waves-effect waves-light">Previous</a>
                                                    </li>
                                                    @endif
                                                    @if($i < count($questions))
                                        ...
                                        @if($loop->last)
                                            <button type="submit" class="btn btn-danger finish_test" id="finish_test">Finish Exam</button>
                                        @endif
                                    @endif
                                        </div>
                                    @endif
